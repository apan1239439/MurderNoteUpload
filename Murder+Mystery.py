...
def get_average_sentence_length(text):
    ...
    avg_words = sum(number_of_words) / len(split_text)
    return avg_words
...
def prepare_text(text):
    cleaned_data = text.lower().replace(".", "").replace("?", "").replace("!", "").split()
    return cleaned_data
...
def build_frequency_table(corpus):
    ...
    for word in corpus:
        if word not in words:
            words.append(word)
    for word in corpus:
        word_count.append(corpus.count(word))   
    frequency_table = {key:value for key, value in zip(words, word_count)} 
    return frequency_table




murderer_frequency_table = build_frequency_table(prepare_text(murder_note))
lily_trebuchet_frequency_table = build_frequency_table(prepare_text(lily_trebuchet_intro))
myrtle_beech_frequency_table = build_frequency_table(prepare_text(myrtle_beech_intro))
gregg_t_fishy_frequency_table = build_frequency_table(prepare_text(gregg_t_fishy_intro))

print(lily_trebuchet_frequency_table)
...
class TextSample:
    ...
    def __str__(self):
        return "The author is {author}. The average sentence length is {average} words.".format(author=self.author, average= self.average_sentence_length )
...
def frequency_comparison(table1, table2):
    ...
    for Word in table1.keys():
        WordsInTable1 = table1[Word]
        WordsInTable2 = table2.get(Word) 
        
        if WordsInTable2:
            apperances += max([WordsInTable1,WordsInTable2])
            mutual_apperances += min([WordsInTable1,WordsInTable2])
            
        else: 
            apperances += WordsInTable1
    
    for Word in table2:
                
        if Word not in table1.keys():
            
            apperances += table2[Word]
            
    
    return mutual_apperances / apperances

            
                
print(frequency_comparison(murderer_frequency_table, lily_trebuchet_frequency_table))                
...
def percent_difference(value1, value2):
    value1 = get_average_sentence_length(value1)
    value2 = get_average_sentence_length(value2)
    return (abs(value1) - abs(value2)) / ((value1 + value2)/2)
...
def find_text_similarity(TextSample1, TextSample2):
    ...
    word_count_similarity = frequency_comparison(build_frequency_table(TextSample1), build_frequency_table(TextSample2))
       
    ngram_similarity = frequency_comparison(build_frequency_table(ngram_creator(TextSample1)), build_frequency_table(ngram_creator(TextSample2)))
...
lily_trebuchet_vs_murderer = find_text_similarity(lily_trebuchet_intro, murder_note)
myrtle_beech_vs_murderer = find_text_similarity(myrtle_beech_intro, murder_note)
gregg_t_fishy_vs_murderer = find_text_similarity(gregg_t_fishy_intro, murder_note)



print("Lily Trebuchet's similarity score is: " + str(lily_trebuchet_vs_murderer) + " % ")
print("Myrtle Beech's similarity score is: " + str(myrtle_beech_vs_murderer) + " % ")
print("Gregg T Fishy's similarity score is: " + str(gregg_t_fishy_vs_murderer) + " % ")
